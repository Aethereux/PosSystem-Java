...
public class WidgetHelper {
    ...
    public static int categoryRow(String[] names, String[] icons, int selected, float animProgress) {
        ...
        float spacing = 8;
        float buttonW = Math.min(100, Math.max(55, (availWidth - spacing * (names.length - 1)) / names.length));
        float buttonH = buttonW * 0.82f;


        int perRow = Math.max(1, (int) ((availWidth + spacing) / (buttonW + spacing)));
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            int col = i % perRow;
            int row = i / perRow;
            float bxLocal = startX + col * (buttonW + spacing);
            float byLocal = startY + row * (buttonH + spacing) + (1 - itemAnim) * 10;
        ...
        int totalRows = (names.length + perRow - 1) / perRow;
        float totalH = totalRows * buttonH + (totalRows - 1) * spacing;
        ImGui.setCursorPos(startX, startY + totalH + 8);
        ImGui.dummy(availWidth, 1);
